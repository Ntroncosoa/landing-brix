...
// ===============================
(function trackAllWhatsAppClicksAsLead() {
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      if (isWhatsAppHref(href)) {
        ...
        return; // deja la navegación normal
      }

      // 2) Botones que NO son <a> pero podrían tener data-href o onclick que lleve a WhatsApp
      const dataHref = el.getAttribute("data-href");
      const onClickAttr = el.getAttribute("onclick") || "";
      const maybe = `${dataHref || ""} ${onClickAttr || ""}`;

      if (isWhatsAppHref(maybe)) {
        safeFbqLead({
          channel: "whatsapp",
          href: dataHref || "",
          text: (el.innerText || el.getAttribute("aria-label") || "")
